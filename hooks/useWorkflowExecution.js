...
import {
  ...
  selectExecution,
  streamDisconnected,
} from "@/lib/store/slices/execution-slice";
...
export function useWorkflowExecution(
  ...
  useEffect(() => {
    executionLockRef.current = isWorkflowRunning;
  }, [isWorkflowRunning]);

  const getPhaseCount = useCallback(
    (label) =>
      agentTemplates.find((item) => item.name === label)?.phases?.length ?? 0,
    [],
  );
    ...
    setNodes((current) =>
      current.map((node) => ({
        ...
        data: {
          ...
          phaseIndex:
            node.data?.status === "Completed" ? node.data?.phaseIndex || 0 : 0,
        },
      })),
    );
  }, [setNodes]);
  ...
  const startWorkflow = useCallback(
    async (startIndex = 0) => {
      ...
      try {
        const validation = await validateWorkflowRequest({
          nodes: runNodes,
          edges,
        }).unwrap();
        if (!validation.valid)
          throw new Error(
            validation.issues?.[0]?.message || "Workflow validation failed.",
          );
  ...
  const stopWorkflow = useCallback(async () => {
    if (execution.runId) {
      try {
        await stopRunRequest({ runId: execution.runId }).unwrap();
      } catch (error) {
        onError?.(error?.message || "Could not stop workflow.");
      }
    }
    executionLockRef.current = false;
  ...
  const handleWorkflowToggle = useCallback(
    (nextState, hasNodes, startIndex = 0) => {
  ...
  const handleRunEvent = useCallback(
    (event) => {
      ...
      if (event.eventId) {
        ...
        if (eventIdsRef.current.size > 250)
      ...
      const nodeIndex = event.nodeId
        ? nodesRef.current.findIndex((node) => node.id === event.nodeId)
        : -1;
      if (event.type === "node.started" && nodeIndex >= 0) {
        const nodeKind = nodesRef.current[nodeIndex]?.data?.nodeKind;
        if (!["start", "end"].includes(nodeKind))
      ...
      if (event.type === "node.queued" && nodeIndex >= 0) {
        setNodes((current) =>
          current.map((node) =>
            node.id === event.nodeId && node.data?.status !== "Running"
      ...
      if (
        (event.type === "stage.started" || event.type === "stage.completed") &&
        nodeIndex >= 0
      ) {
        setNodes((current) =>
          current.map((node) =>
            node.id === event.nodeId
              ? {
                  ...
                  data: {
                    ...node.data,
                    phaseIndex: event.stageIndex,
                    totalPhases: event.totalStages,
                  },
                }
              : node,
          ),
            ...
            node.id === event.nodeId
              ? {
                  ...
                  data: {
                    ...
                    phaseIndex:
                      node.data?.totalPhases || node.data?.phaseIndex || 0,
                  },
                }
              : node,
          ),
      ...
      if (event.type === "node.failed" && nodeIndex >= 0) {
        setNodes((current) =>
          current.map((node) =>
            node.id === event.nodeId
              ? {
                  ...node,
                  data: { ...node.data, status: "Failed", error: event.error },
                }
              : node,
          ),
        );
        appendNodeLog(
          event.nodeId,
          event.error?.message || "Agent execution failed.",
          "ERROR",
          event.timestamp,
        );
      }
  ...
  const retryNode = useCallback(
    async (nodeId) => {
      ...
      try {
        if (execution.runId)
          await retryNodeRequest({ runId: execution.runId, nodeId }).unwrap();
  ...
  const executionIndex = execution.activeNodeId
    ? nodes.findIndex((node) => node.id === execution.activeNodeId)
    : -1;
  return {
    executionIndex,
    nodePhaseProgress: execution.phaseProgress,
    handleWorkflowToggle,
    retryNode,
    activeRun: execution.runId
      ? {
          runId: execution.runId,
          eventsUrl: execution.eventsUrl,
