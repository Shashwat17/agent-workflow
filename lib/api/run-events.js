...
function subscribeToMockRun(runId, onEvent, onError) {
  ...
  const execute = async () => {
    ...
    if (run.startNode) {
      emit("node.started", { nodeId: run.startNode.id, status: "running", attempt: 1, startedAt: new Date().toISOString() });
      await wait(250);
      emit("node.completed", { nodeId: run.startNode.id, status: "completed", output: "", attempt: 1, completedAt: new Date().toISOString() });
    }
    ...
    if (run.endNode) {
      emit("node.started", { nodeId: run.endNode.id, status: "running", attempt: 1, startedAt: new Date().toISOString() });
      await wait(250);
      emit("node.completed", { nodeId: run.endNode.id, status: "completed", output: "", attempt: 1, completedAt: new Date().toISOString() });
    }
