...
import { getExecutionOrder, validateWorkflow } from "@/lib/workflow-validation";
...
const fail = (config, code, message, status = 400) => ({
  data: {
    success: false,
    error: { code, message, requestId: createMockId("req") },
  },
  status,
  statusText: "Error",
  headers: {},
...
export async function mockAdapter(config) {
  ...
  if (method === "get" && path === "/agents")
    return ok(config, { agents: agentTemplates });
  if (method === "post" && path === "/workflows/validate") {
    ...
    return ok(config, {
      ...
      issues,
      executionOrder: getExecutionOrder(body.nodes || [], body.edges || []).map(
        (node) => node.id,
      ),
    });
  }
  if (method === "post" && path === "/runs") {
    ...
    let orderedAgents = getExecutionOrder(allNodes, body.edges || []);
    if (body.startNodeId) {
      const retryIndex = orderedAgents.findIndex(
        (node) => node.id === body.startNodeId,
      );
      if (retryIndex >= 0) orderedAgents = orderedAgents.slice(retryIndex);
    }
    const startNode = allNodes.find((node) => node.data?.nodeKind === "start");
    const endNode = allNodes.find((node) => node.data?.nodeKind === "end");
    ...
    return ok(
      config,
      {
        ...
        status: "queued",
        createdAt: new Date().toISOString(),
        executionOrder: orderedAgents.map((node) => node.id),
  ...
  if (method === "get" && runMatch) {
    ...
    return run
      ? ok(config, run)
      : fail(config, "RUN_NOT_FOUND", "Run was not found.", 404);
  }
  ...
  if (method === "delete" && fileMatch) {
    mockStore.files.delete(fileMatch[1]);
    return ok(config, { fileId: fileMatch[1], deleted: true });
  }
  ...
  if (method === "get" && path === "/connections")
    return ok(config, { connections: mockStore.connections });
  if (method === "post" && path === "/connections/test") {
    const { token: _token, ...safeBody } = body;
    void _token;
    const label =
      safeBody.label || safeBody.name || safeBody.id || "Connection";
    return ok(
      config,
      {
        id: safeBody.id || createMockId("connection"),
        status: "connected",
        message: `${label} connection test succeeded.`,
      },
      200,
    );
  }
  if (method === "post" && path === "/connections") {
    ...
    const connection = {
      ...safeBody,
      id: body.id || createMockId("connection"),
      status: "Connected",
    };
    mockStore.connections = [
      ...mockStore.connections.filter((item) => item.id !== connection.id),
      connection,
    ];
  ...
  if (method === "delete" && connectionMatch) {
    mockStore.connections = mockStore.connections.filter(
      (item) => item.id !== connectionMatch[1],
    );
    return ok(config, { connectionId: connectionMatch[1], deleted: true });
  }
  return fail(
    config,
    "MOCK_ROUTE_NOT_FOUND",
    `No mock response for ${method.toUpperCase()} ${path}.`,
    404,
  );
}
