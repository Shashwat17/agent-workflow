...
export async function mockAdapter(config) {
  ...
  if (method === "post" && path === "/workflows/validate") {
    ...
    return ok(config, { valid: !issues.some((issue) => issue.type === "error"), issues, executionOrder: getExecutionOrder(body.nodes || [], body.edges || []).map((node) => node.id) });
  }
  if (method === "post" && path === "/runs") {
    ...
    if (body.startNodeId) {
      const retryIndex = orderedAgents.findIndex((node) => node.id === body.startNodeId);
    ...
    mockStore.runs.set(runId, { runId, status: "queued", createdAt: new Date().toISOString(), nodes: orderedAgents, startNode, endNode, edges: body.edges || [], stopped: false });
    return ok(config, { runId, status: "queued", createdAt: new Date().toISOString(), executionOrder: orderedAgents.map((node) => node.id), eventsUrl: `/runs/${runId}/events`, statusUrl: `/runs/${runId}` }, 202);
  }
